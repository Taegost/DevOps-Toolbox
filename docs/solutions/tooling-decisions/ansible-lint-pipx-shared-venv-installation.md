...
- `pipx runpip ansible install ansible-lint==${ANSIBLE_LINT_VERSION}` is raw `pip` executed *inside* the venv, so the package shares one `ansible-core` with runtime.
...
- Append `pipx runpip ansible check` to the **same** `RUN` as the install. This is the guard against silent resolver drift: it verifies the venv's declared requirements (Galaxy collection Python deps, `python-ansible-requirements.txt` pins) are still intact, and fails the build if not. On success it prints `No broken requirements found.`
...
- Use a **separate** `pipx install` venv instead when the constraints conflict and cannot be reconciled, or when the tool would force upgrades to runtime-critical packages that `pipx runpip ansible check` cannot tolerate. Isolation beats sharing when sharing means breakage; the check step is the arbiter.
- Also applies when updating either package: after any bump of `ANSIBLE_VERSION` or `ANSIBLE_LINT_VERSION`, both `pipx runpip ansible check` and the lockstep-coupling rule come into play (ansible-lint tracks the last two Ansible majors).

## Examples

**Failure modes the guard catches.** With `pipx runpip ansible check` in the same `RUN`, a resolver drift — say ansible-lint requiring a newer `pyyaml` than a pinned collection dep allows, or an upgrade breaking an azure-collection requirement — fails the build with a broken-requirements error instead of silently producing a drifted image. Without the check, the same install exits 0 with a warning and the drift ships.
...
- `pipx runpip ansible check` → `No broken requirements found.` (no resolver drift).
