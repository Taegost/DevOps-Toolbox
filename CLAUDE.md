...
5. Azure CLI extension index (`az extension add --system`, ssh extension) — `--system` installs under the CLI's lib path (`/opt/az/...`) so it survives the `USER vscode` switch
